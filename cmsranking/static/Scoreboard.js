...
<tr class=\"user\" data-user=\"" + user["key"] + "\"> \
    ...
    <td colspan=\"10\" class=\"l_name\">" + user["l_name"] + "</td>";
        ...
        if (user['team']) {
            result += " \
    <td class=\"team\"><img src=\"img/" + user["team"] + ".png\" title=\"" + DataStore.teams[user["team"]]["name"] + "\" /></td>";
        } else {
            result += " \
    <td class=\"team\"></td>";
        }
    ...
    self.update_user = function (u_id, old_user, user) {
        ...
        if (user["team"]) {
            $row.children(".team").html("<img src=\"img/" + user["team"] + ".png\" title=\"" + DataStore.teams[user["team"]]["name"] + "\" />");
        } else {
