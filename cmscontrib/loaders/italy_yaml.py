...
class YamlLoader(ContestLoader, TaskLoader, UserLoader, TeamLoader):
    ...
    def get_task_loader(self, taskname):
        return type(self)(os.path.join(self.path, taskname), self.file_cacher)
    ...
    def get_task(self, get_statement=True) -> Task | None:
        ...
        if get_statement:
            ...
            for lang, lang_code in LANGUAGE_MAP.items():
                path = os.path.join(self.path, statement, "statement.%s.pdf" % lang_code)
        ...
        # Otherwise fall back on the old one.
            ...
            if conf.get("token_initial", None) is not None:
                logger.warning(
        ...
        for lang in LANGUAGES:
            if os.path.exists(os.path.join(
                    self.path, sol_dir, "grader%s" % lang.source_extension)):
                graders = True
                break
        if graders:
            ...
            for lang in LANGUAGES:
                ...
                grader_filename = os.path.join(
                    self.path, sol_dir, "grader%s" % extension)
            ...
            for other_filename in os.listdir(os.path.join(self.path, sol_dir)):
                if any(other_filename.endswith(header)
                       for header in HEADER_EXTS):
                    digest = self.file_cacher.put_file_from_path(
                        os.path.join(self.path, sol_dir, other_filename),
        ...
        if args["score_type"] not in SCORE_TYPES:
            logger.warning(f'Task has unknown score type "{args["score_type"]}"!')
                ...
                if os.path.exists(path):
                    ...
                    for lang in LANGUAGES:
                        stub_name = os.path.join(
                            self.path, sol_dir, "stub%s" % lang.source_extension)
    ...
    def task_has_changed(self):
        ...
        for lang in LANGUAGE_MAP.values():
            files.append(os.path.join(self.path, "statement", "statement.%s.pdf" % lang))
        ...
        if not conf.get('output_only', False) and \
                os.path.isdir(os.path.join(self.path, sol_dir)):
            for lang in LANGUAGES:
